"""
kakao.py - Kakao i Open Builder webhook receiver + Make.com 연동
"""
...
async def make_incoming(
    ...
    parent = db.query(Parent).filter(Parent.kakao_user_id == body.kakao_user_id).first()
    if not parent:
        raise HTTPException(
            status_code=404,
            detail=(
    ...
    saved_path = await save_uploaded_image(str(body.image_url))
    if not saved_path:
        raise HTTPException(
            status_code=400,
    ...
    submission = Submission(
        parent_id=parent.id,
        photo_path=serialize_photo_paths([saved_path]),
        level=parent.level,
        stage=None,
        extra_instruction=body.message,
        feedback_draft=None,
        status="pending",
    )
    db.add(submission)
